...
class KMeans:
    ...
    def proces(self, sample):
        dista = dist(sample, self._centroides[0])
        match = 0
        for i in range(1, len(self._centroides)):
            new_dist = dist(sample, self._centroides[i]) 
            if(new_dist < dista):  
                match = i
                dista = new_dist
        return match
    ...
    def train(self, samples, n_clusters, max_iter):
        ...
        """
        if len(samples) == 2:
            self._centroides.append([2, 1])
            self._centroides.append([8, 10])
        """
...
class KMeansTest(unittest.TestCase):
    def setUp(self):
        ...
        self.twoFeaturesSamples = [[1, 2, 9, 11], [1, 2, 9, 11]]
        self.oneCluster = 1
        self.twoClusters = 2
    
    def testKMeansTrainDevuelveUnCentroidePara1FeatureY1Clase(self):
        self.km = KMeans()
        self.assertAlmostEqual(2, self.km.train(self.oneFeatureSamples, self.oneCluster, 0)[0][0])        
    ...
    def testKMeansElCentroidePara1FeatureY1ClaseEsCorrectoLuegoDeEntrenar(self):
        ...
        self.assertAlmostEqual(2, self.km.centroides()[0][0])    
    ...
    def testKMeansFuncionaPara1FeatureY2ClasesUsando4Samples(self):
        ...
        self.assertAlmostEqual(0, self.km.proces([3])) 
        self.assertAlmostEqual(1, self.km.proces([10])) 
        self.assertAlmostEqual(1.5, self.km.centroides()[0][0])
        self.assertAlmostEqual(10, self.km.centroides()[1][0])
    ...
    def testKMeansFuncionaPara1FeatureY2ClasesUsando4SamplesYConDosIteraciones(self):
        ...
        self.assertAlmostEqual(10, self.km.centroides()[1][0])
    """
    def testKMeansFuncionaPara2FeatureY2Clases(self):
        self.km = KMeans()
        self.km.train(self.twoFeaturesSamples, self.twoClusters, 2)
        self.assertAlmostEqual(0, self.km.proces([3, 2]))
        self.assertAlmostEqual(1, self.km.proces([9, 10])) 
    """
